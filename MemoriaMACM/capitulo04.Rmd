...
```{r echo=FALSE,out.width='8cm',fig.align='center',fig.cap="\\label{fig:ModMetodologia}Pasos que se llevaran a cabo en el modelo. \\newline{}Fuente: elaboraci\\'on propia",fig.pos="H"}
knitr::include_graphics("figurasR/modelo_metodologia.png")
```
...
* Rvest: Está diseñado para trabajar junto a magrittr para expresar operaciones complejas de manera elegante y fácil de entender. 
* robots.txt: Permite averiguar si las secciones de la página web son legalmente accesible. Si sale disallow es que son accesibles.
*	Readr: Permite leer el fichero cvs.
*	Xml2: Permite analizar documentos XML 
*	Tydiverse: Es un conjunto de paquetes en R diseñados para ciencia de datos. Entre los paquetes que incluye, Se utilizarán:
  + Dplyr: Ayuda a la manipulación de datos, permite mutar o crear nuevas variables, seleccionar, filtrar, sumarizar, acomodar o agrupar las variables que se tengan en las tablas.
  + Ggplot2: Es un paquete que se utiliza para la visualización de los datos mediante gráficos.
  ...
  + Purrr: Este paquete facilita el trabajo con vectores y funciones.
  + Tibble: Viene a ser una transformación del data frame. Aprovecha lo bueno de los data frame y mejora algunas opciones que quedaron obsoletas en versiones anteriores.
  + Stringr: Ayuda a la preparación y limpieza de los datos.
  + Forcats: Es un paquete especial para lidiar con factores y datos categóricos.
*	Libxml2: Es una biblioteca de código para analizar documentos XML
*Psych: Es un paquete que se utiliza para la creación de gráficos de correlación 

Asimismo, dentro de Rtudio se utilizó una herramienta denominada R Markdown para llevar a cabo la elaboración del informe.
...
R Markdown es una herramienta ofimática que está diseñado con el fin de facilitar la creación de contenido y a su vez muchos tipos de salida (HTML, PDF, MS_Word, ODT, RTF, entre otros) a partir del archivo .Rmd. el cual permite combinar código en lenguaje R y contenido en formato de texto. Así mismo, permite:

*	Automatizar tareas, manipular y analizar datos. 
...
```{r echo=FALSE,out.width='8cm',fig.align='center',fig.cap="\\label{fig:RMarkdown}Pasos que se llevaran a cabo en el modelo. \\newline{}Fuente: --------------------- ",fig.pos="H"}
knitr::include_graphics("figurasR/PagWeb4.png")
```
