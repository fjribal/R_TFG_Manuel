...
date: "2/08/2022"
title: "Capitulo06"
subtitle: "Limpieza de la base de datos (detección de datos anómalos)"
documentclass: book
...
Como segunda alternativa, se utilizó la desviación absoluta alrededor de la mediana (M) al ser la mediana al igual que la media, una medida de tendencia central que apenas se ve afectado por la presencia de valores atípicos. El MAD se calculó utilizando la siguiente formula:

$$
MAD = b * median (|x_i-median(x)|
$$

Con este valor se procedió a calcular el umbral conservador (∝ =3) para la detección de valores anómalos: 

$$
T_{min},T_{max}=median(x)±(∝ * MAD)
$$
...
Una vez culminada la limpieza de datos con ambos métodos, se observaron mejores resultados con el segundo método, produciendo una reducción del total de la muestra a `r nrow(data_no_outlier_Mad)`cabezas tractoras.
